								  ...
								  char* inf_name, struct wdi_options_prepare_driver* options)
{
	...
	if ( (device_info->desc == NULL) 
	  && ((options->generic_driver) && (device_info->device_id == NULL)) ) {
		wdi_err("no description or device ID was given for the device - aborting");
		MUTEX_RETURN WDI_ERROR_INVALID_PARAM;
	}
	...
	if (options->generic_driver) {
		static_strcpy(inf_entities[DEVICE_HARDWARE_ID].replace, device_info->device_id);
	...
	if ( (windows_version >= WINDOWS_VISTA) && IS_VISTA_SHELL32_AVAILABLE && (pIsUserAnAdmin()) )  {
		...
		// the DEVICE_HARDWARE_ID is either "VID_####&PID_####[&MI_##]" or the device_id provided
		sprintf(hw_id, "USB\\%s", options->generic_driver?
			device_info->device_id:inf_entities[DEVICE_HARDWARE_ID].replace);
